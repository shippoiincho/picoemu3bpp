...
秋葉原では[マルツで入手できる](https://www.marutsu.co.jp/pc/i/838015/)ようです。

- 圧電ブザー(圧電スピーカ)

![Buzzer](picoemu01.jpg)

音を出すためのブザーです。同じく[秋月電子などで入手可能](https://akizukidenshi.com/catalog/g/g101251/)です。
...
- Pico を取り外したい場合には、別途ピンソケットを購入してピンソケット経由で取り付けてください。
...
ここまで正しく動作していない場合は、ファームウェアの書き込みや ROM イメージの書き込みに失敗していますので、
やりなおしてください。
...
NEW-MONITOR が画面で `L` (+enter) と入力します。
エミュレータがテープからロードするモードに入りますので、メニューの `LOAD CMT` を選択します。
...
- 音声出力はブザーを前提にしていますが、外部に追加した LPF 回路を経由してライン入力に接続することが可能です。

- ケースなどは 
を使うことが可能です。
1/3 サイズで作られていますので余裕をもって内蔵できると思います。
